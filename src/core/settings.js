  ...
  Storage.get(
    ...
    (result) => {
      ...
      document.getElementById("engine-select").value =
        settings.engine || Storage.defaults.engine;

      document.getElementById("youtube-prompt").value =
        settings.youtubePrompt || Storage.defaults.youtubePrompt;

      document.getElementById("twitter-prompt").value =
        settings.twitterPrompt || Storage.defaults.twitterPrompt;

      document.getElementById("thread-prompt").value =
        settings.threadPrompt || Storage.defaults.threadPrompt;

      document.getElementById("webpage-prompt").value =
        settings.webpagePrompt || Storage.defaults.webpagePrompt;

      document.getElementById("selection-prompt").value =
        settings.selectionPrompt || Storage.defaults.selectionPrompt;
    },
  );
